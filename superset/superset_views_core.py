...
class Superset(BaseSupersetView):
    ...
    def dashboard(self, dashboard_id):
        ...
        print('in /dashboard/<dashboard_id>/: %s', request.path)
        if config.get('ENABLE_ACCESS_REQUEST'):
            for datasource in datasources:
                if datasource and not security_manager.datasource_access(datasource):
                    ...
                    print('inside if /dashboard/<dashboard_id>/: %s | %s', localpath, request.path)
                    localmessages = request.args.get('messages',{})
                    return redirect(url_for("Superset.request_access", dashboard_id=str(dash.id), next=localpath))
