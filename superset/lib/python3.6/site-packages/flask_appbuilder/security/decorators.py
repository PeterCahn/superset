...
#        return redirect(url_for(self.appbuilder.sm.auth_view.__class__.__name__ + ".login"))
        print("has_access => path requested is this: ", request.path)
        return redirect(url_for(self.appbuilder.sm.auth_view.__class__.__name__ + ".login", next=request.args.get('next', request.path)))
