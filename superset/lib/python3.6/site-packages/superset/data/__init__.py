...
def get_or_create_custom_db(db_name):
    from superset import conf, db
    from superset.models import core as models

    dbobj = db.session.query(models.Database).filter_by(database_name=db_name).first()
    if not dbobj:
        dbobj = models.Database(database_name=db_name)
...
uri = os.environ.get('SQLALCHEMY_DATA_URI','sqlite:////var/lib/superset/superset.db')
if uri is 'sqlite:////var/lib/superset/superset.db':
    target_engine = db.engine
    logger.debug('target_engine sqlite: {0}'.format(target_engine))
else:
    url = make_url(uri)
    target_engine = create_engine(url)
    logger.debug('target_engine: {0}'.format(target_engine))
...
def load_energy():
    ...
    tbl.database = utils.get_or_create_main_db() if uri == 'sqlite:////var/lib/superset/superset.db' else get_or_create_custom_db('DataDB')
...
def load_world_bank_health_n_pop():
    ...
    if make_url(uri).drivername == 'postgresql':
        from superset.data.fix_examples import fix_tables
        print('Applying fixes to \'{0}\''.format(tbl_name))
        fix_tables()
...
def load_birth_names():
    ...
    obj.database = utils.get_or_create_main_db() if uri == 'sqlite:////var/lib/superset/superset.db' else get_or_create_custom_db('DataDB')
