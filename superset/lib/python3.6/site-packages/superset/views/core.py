            ...
            for datasource in datasources:
                if datasource and not security_manager.datasource_access(datasource):
                    flash(
                        __(security_manager.get_datasource_access_error_msg(datasource)),
                        'danger')
                    if os.environ.get('SHIBS_HEADERS') is None :
                        return redirect('superset/request_access/?''dashboard_id={dash.id}&'.format(**locals()))
                    else :
                        # SHIBS_HEADERS has been set, we plan to access through a ProxyPass and Shibboleth
